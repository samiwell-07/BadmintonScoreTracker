...
describe('Scoreboard', () => {
  ...
  it('vibrates for point and completed swipe actions only', async () => {
    const user = userEvent.setup()
  ...
  it('disables haptics from General settings and keeps them off after remount', async () => {
    const user = userEvent.setup()
    const vibrate = vi.fn(() => true)
    const originalVibrate = Object.getOwnPropertyDescriptor(
      navigator,
    ...
    try {
      const firstRender = render(<Scoreboard />)
      await user.click(
        screen.getByRole('button', { name: 'Open center menu' }),
      )
      await user.click(
        screen.getByRole('button', { name: 'Match settings' }),
      )
      await user.click(
